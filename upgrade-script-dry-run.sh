...
# Autoremove script
echo -e "\nThe following needs to be REMOVED:" >> /var/log/upgrades-script.log && apt-get -s autoremove | awk 'BEGIN{counter=0} /^The following packages will be REMOVED:$/ {counter++; next;} /^[0-9]+ upgraded/ {exit;} counter>0 {gsub(/^ +/, "", $0); print;}' >> /var/log/upgrades-script.log
